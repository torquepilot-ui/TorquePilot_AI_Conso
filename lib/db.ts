...
function normalizeConnectorRecord(record: any, connector: UsageConnector, sourceName: string, fallbackDate: string): ParsedUsageCandidate | null {
  if (!record || typeof record !== "object") return null;
  if (connector === "generic" || connector === "hermes") return normalizeUsageRecord(record, sourceName, fallbackDate);
  const specs: Record<Exclude<UsageConnector, "generic" | "hermes">, { label: string; input: string[]; output: string[]; cache: string[]; reasoning: string[]; id: string[]; date: string[]; method: string; forceZeroCost?: boolean }> = {
