  ...
  return <main className="shell">
    ...
    <section className="layout usageLayout">
      ...
      <aside className="panel"><div className="sectionHeader"><div><p className="eyebrow">Collecteur local Phase 4I</p><h2>Import guidé</h2></div><span className="pill">{collectorPreview.totals.readyFiles ? `${collectorPreview.totals.readyFiles} prêt(s)` : "OK"}</span></div>
        ...
        <details className="row compact collectorDetails"><summary><div><h3>Dossiers acceptés</h3><p>{collectorPreview.folders.join(" · ")}</p></div><span className="pill formatPill">JSON/JSONL/log/txt</span></summary><p className="muted">Aucune clé API demandée : uniquement fichiers ou logs locaux déjà présents sur le Lenovo.</p></details>
